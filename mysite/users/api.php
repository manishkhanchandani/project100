...
$rsUserExist = mysql_query($query_rsUserExist, $conn_mysite) or die(mysql_error());
...
$totalRows_rsUserExist = mysql_num_rows($rsUserExist);
		...
		if ($totalRows_rsUserExist > 0) {
			...
			$updateSQL = sprintf("UPDATE users_auth SET display_name=%s, profile_img=%s, email=%s, logged_in_time=%s WHERE user_id=%s",
			  ...
			  mysql_select_db($database_conn_mysite, $conn_mysite);
			  $Result1 = mysql_query($updateSQL, $conn_mysite) or die(mysql_error());
		}
		
		// THE USER WILL NOT BE IN MY DATABASE - NO
		if ($totalRows_rsUserExist == 0) {
			//INSERT THE USER IN THE DATABASE
			$refid = !empty($_COOKIE['refid']) ? $_COOKIE['refid'] : 1;
			 $insertSQL = sprintf("INSERT INTO users_auth (display_name, profile_img, email, provider_id, user_created_dt, uid, logged_in_time, profile_uid, ref_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       GetSQLValueString($userData['displayName'], "text"),
                       GetSQLValueString($userData['photoURL'], "text"),
                       GetSQLValueString($userData['email'], "text"),
