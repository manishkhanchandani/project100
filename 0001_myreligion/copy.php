...
if (isset($_SESSION['MM_UserId'])) {
  $colname_rsReligion = (get_magic_quotes_gpc()) ? $_SESSION['MM_UserId'] : addslashes($_SESSION['MM_UserId']);
}
mysql_select_db($database_conn, $conn);
$query_rsReligion = sprintf("SELECT * FROM religions WHERE user_id = %s", $colname_rsReligion);
...
//custom code
if (!empty($_POST)) { //that means the form is submitted
	$sql = sprintf("insert into religions_view SET view_user_id = %s, religion_id = %s, view_description = %s, category_id = %s, view_created_dt = %s, view_status = %s, view_images = %s, view_videos = %s, view_links = %s, from_religion_id = %s, from_verse_id = %s",
		GetSQLValueString($_SESSION['MM_UserId'], "int"),
		GetSQLValueString($_POST['religion_id'], "int"),
		GetSQLValueString($row_rsVerse['view_description'], "text"),
		GetSQLValueString($row_rsVerse['category_id'], "int"),
		GetSQLValueString(date('Y-m-d H:i:s'), "date"),
		GetSQLValueString($row_rsVerse['view_status'], "int"),
		GetSQLValueString($row_rsVerse['view_images'], "text"),
		GetSQLValueString($row_rsVerse['view_videos'], "text"),
		GetSQLValueString($row_rsVerse['view_links'], "text"),
...
<div class="container">
	<h3>Copy Verse To My Relgion</h3>
	<form name="form1" method="post" action="">
	  <strong>Religion Where You Want to Copy:</strong> 
	  <label>
	  <select name="religion_id">
	    <?php
do {  
?>
	    <option value="<?php echo $row_rsReligion['religion_id']?>"><?php echo $row_rsReligion['religion_name']?></option>
	    <?php
} while ($row_rsReligion = mysql_fetch_assoc($rsReligion));
  $rows = mysql_num_rows($rsReligion);
  if($rows > 0) {
      mysql_data_seek($rsReligion, 0);
	  $row_rsReligion = mysql_fetch_assoc($rsReligion);
  }
?>
	  </select>
	  ...
	  <p>&nbsp;</p>
	  <p><strong>Verse To Copy: </strong><?php echo $row_rsVerse['view_description']; ?></p>
	  <p>&nbsp;</p>
	  <p>
		<label>
